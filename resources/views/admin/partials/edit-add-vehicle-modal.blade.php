<div x-show="addModalOpen || editModalOpen" 
    ...
    <div @click.away="addModalOpen = false; editModalOpen = false" 
         class="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <form :action="currentVehicle.id ? '/admin/vehicles/' + currentVehicle.id : '{{ route('admin.vehicles.store') }}'" 
            ...
            <div class="space-y-3 pt-2">
                ...
                <div class="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                    ...
                    <input type="file" name="gallery[]" x-ref="galleryInput" class="hidden" multiple @change="handleGalleryChange">
                </div>
            </div>

            <div class="space-y-2">
                <label class="text-sm font-bold text-slate-700 ml-1">Description</label>
                <textarea name="description" x-model="currentVehicle.description" rows="3" 
                          :placeholder="currentVehicle.id ? '' : 'Describe the condition, engine, etc...'"
                          class="w-full bg-gray-50 border-none rounded-3xl p-4 text-sm font-medium focus:ring-2 focus:ring-slate-900"></textarea>
            </div>

            <div class="flex gap-4 pt-4 sticky bottom-0 bg-white pb-2">
                <button type="button" @click="addModalOpen = false; editModalOpen = false" 
                        class="flex-1 py-4 bg-gray-50 text-slate-900 rounded-2xl font-bold hover:bg-gray-100 transition-all">
                    Cancel
                </button>
                <button type="submit" 
